...
export default function Home() {
  ...
  const storedName = readString("name", defaults.name);
  const storedPhoto = readString("photo", defaults.photo);
  const storedInterests = readList("interests", defaults.interests);
  const storedPlans = readList("plans", defaults.plans);
...
/* Save profile edits */
  const onSave = () => {
    ...
    let currentPhoto = defaults.photo;
    try {
      const raw = localStorage.getItem("photo");
      if (raw && raw.trim()) currentPhoto = raw;
    } catch {}
